...
def _pick_chess_piece(
    ...
    if grasp_setup_ee_position_aux[0] * grasp_setup_ee_position_aux[1] > 0:
        grasp_setup_robot_arm_configuration = FRONT_PICK_AND_PLACE_SETUP_ROBOT_ARM_CONFIGURATION_RIGHT_ARM
    else: 
        grasp_setup_robot_arm_configuration = FRONT_PICK_AND_PLACE_SETUP_ROBOT_ARM_CONFIGURATION_LEFT_ARM
...
def _place_chess_piece(
    ...
    if release_setup_ee_position_aux[0] * release_setup_ee_position_aux[1] > 0:
        release_setup_robot_arm_configuration = FRONT_PICK_AND_PLACE_SETUP_ROBOT_ARM_CONFIGURATION_RIGHT_ARM
    else: 
        release_setup_robot_arm_configuration = FRONT_PICK_AND_PLACE_SETUP_ROBOT_ARM_CONFIGURATION_LEFT_ARM
    ...
    if grasping:
        ...
        chess_piece_link_name = chess_piece_model_name + '::link'
        robot_arm.gripper.command(OPEN_GRIPPER_POSITION) 
...
@timeout(seconds=15, default=False)
def _move_chess_piece(
        robot_arm, 
        chess_piece_type, 
        starting_cell, 
        final_cell, 
        grasp_positions_z_dict=DEFAULT_GRASP_POSITIONS_Z_DICT,
        grasp_gripper_positions_dict=DEFAULT_GRASP_GRIPPER_POSITIONS_DICT, 
        grasping=True, 
        wait=True, 
        duration=1.0
    ):
    ...
    robot_arm.set_joint_positions(position=REST_ROBOT_ARM_CONFIGURATION, wait=wait, t=duration)
    return True
