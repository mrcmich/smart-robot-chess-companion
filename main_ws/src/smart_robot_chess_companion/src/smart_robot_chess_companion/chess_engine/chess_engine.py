...
class GameState:
    """Stores the state of the chess game, prints the chess board, finds valid moves, stores move logs."""
    ...
    def print_board(self):
        ...
        rospy.logwarn("  a b c d e f g h")
        for row in range(8):
            row_str = str(8 - row) + " "
            for col in range(8):
                piece = self._board[7 - row][col]
                if piece is Player.EMPTY:
                    row_str += ". "
                else:
                    row_str += piece.get_name() + " "
            rospy.logwarn(row_str)
                ...
                for move in initial_valid_piece_moves:
                    ...
                    for piece in checking_pieces:
                        if moving_piece.get_name() == "k":
                            temp = self._board[current_row][current_col]
                            self._board[current_row][current_col] = Player.EMPTY
                            temp2 = self._board[move[0]][move[1]]
                            self._board[move[0]][move[1]] = temp
                            if not self.check_for_check(move, moving_piece.get_player())[0]:
                                pass
                            else:
                                can_move = False
                            self._board[current_row][current_col] = temp
                            self._board[move[0]][move[1]] = temp2
                        elif move == piece and len(checking_pieces) == 1 and moving_piece.get_name() != "k" and \
                                (current_row, current_col) not in pinned_pieces:
                            pass
                        elif move != piece and len(checking_pieces) == 1 and moving_piece.get_name() != "k" and \
                                (current_row, current_col) not in pinned_pieces:
                            temp = self._board[move[0]][move[1]]
                            self._board[move[0]][move[1]] = moving_piece
                            self._board[current_row][current_col] = Player.EMPTY
    ...
    def promote_pawn(self, starting_square, moved_piece, ending_square):
        while True:
            ...
            if new_piece_name in piece_classes:
                ...
                self._board[ending_square[0]][ending_square[1]] = new_piece
                self._board[moved_piece.get_row_number()][moved_piece.get_col_number()] = Player.EMPTY
    ...
    # TODO: have to fix en passant for AI
            ...
            if ending_square in valid_moves:
                ...
                if moving_piece.get_name() == "k":
                    if moving_piece.is_player(Player.PLAYER_1):
                        if moved_to_piece == Player.EMPTY and next_square_col == 1 and self.king_can_castle_left(
                            ...
                            self._board[0][2] = self._board[0][0]
                            self._board[0][0] = Player.EMPTY
                            ...
                            self._board[0][4] = self._board[0][7]
                            self._board[0][7] = Player.EMPTY
                        ...
                        if moved_to_piece == Player.EMPTY and next_square_col == 1 and self.king_can_castle_left(
                            ...
                            # move rook
                            self._board[7][2] = self._board[7][0]
                            self._board[7][0] = Player.EMPTY
                            ...
                            # move rook
                            self._board[7][4] = self._board[7][7]
                            self._board[7][7] = Player.EMPTY
                ...
                # Add move class here
                    ...
                    # en passant
                        ...
                        if moving_piece.is_player(Player.PLAYER_1):
                            move = ChessMove(starting_square, ending_square, self, self._is_check)
                            move.en_passant_move(self._board[next_square_row - 1][next_square_col],
                                                 (next_square_row - 1, next_square_col))
                            self.move_log.append(move)
                            self._board[next_square_row - 1][next_square_col] = Player.EMPTY
                        else:
                            move = ChessMove(starting_square, ending_square, self, self._is_check)
                            move.en_passant_move(self._board[next_square_row + 1][next_square_col],
                                                 (next_square_row + 1, next_square_col))
                ...
                if temp:
                    ...
                    self._board[next_square_row][next_square_col] = self._board[current_square_row][current_square_col]
                    self._board[current_square_row][current_square_col] = Player.EMPTY
    ...
    def undo_move(self):
        if self.move_log:
            ...
            if undoing_move.castled is True:
                self._board[undoing_move.starting_square_row][
                    undoing_move.starting_square_col] = undoing_move.moving_piece
                self._board[undoing_move.ending_square_row][undoing_move.ending_square_col] = undoing_move.removed_piece
                ...
                self._board[undoing_move.rook_starting_square[0]][
                    undoing_move.rook_starting_square[1]] = undoing_move.moving_rook
                self._board[undoing_move.rook_ending_square[0]][undoing_move.rook_ending_square[1]] = Player.EMPTY
                ...
                self._board[undoing_move.en_passant_eaten_square[0]][
    ...
    def check_for_check(self, king_location, player):
        ...
        while king_location_col - _left >= 0 and self.get_piece(king_location_row,
            ...
            if self.is_valid_piece(king_location_row, king_location_col - _left) and \
                ...
                if _possible_pin:
                    temp = self._board[_possible_pin[0]][_possible_pin[1]]
                    self._board[_possible_pin[0]][_possible_pin[1]] = Player.EMPTY
                    if (king_location_row, king_location_col) in self.get_piece(king_location_row,
                                                                                king_location_col - _left).get_valid_piece_moves(self):
                        _pins.append(_possible_pin)
                        _pins_check.append((king_location_row, king_location_col - _left))
                    self._board[_possible_pin[0]][_possible_pin[1]] = temp
                else:
                    if (king_location_row, king_location_col) in self.get_piece(king_location_row,
                                                                                king_location_col - _left).get_valid_piece_moves(self):
                        # self._is_check = True
        ...
        while king_location_col + _right < 8 and self.get_piece(king_location_row,
            ...
            if self.is_valid_piece(king_location_row, king_location_col + _right) and \
                ...
                if _possible_pin:
                    ...
                    self._board[_possible_pin[0]][_possible_pin[1]] = Player.EMPTY
                    if (king_location_row, king_location_col) in self.get_piece(king_location_row,
                                                                                king_location_col + _right).get_valid_piece_moves(self):
                        _pins.append(_possible_pin)
            ...
            if self.is_valid_piece(king_location_row + _down, king_location_col) and \
                ...
                if _possible_pin:
                    ...
                    if (king_location_row, king_location_col) in self.get_piece(king_location_row + _down,
                                                                                king_location_col).get_valid_piece_moves(self):
                        _pins.append(_possible_pin)
